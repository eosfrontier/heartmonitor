...
static inline int i2c_update_reg(int i2c, int reg, uint8_t mask, uint8_t val)
{
    uint8_t curval;
    int res;
    res = i2c_ioctl(i2c, 0, reg, 1, &curval);
    if (res < 0) { return res; }
    val = (curval & (mask ^ 0xff)) | (val & mask);
    return i2c_ioctl(i2c, 1, reg, 1, &val);
}

static int set_mode(max30102_t *mx, uint8_t mode)
{
    return i2c_ioctl(mx.i2c, 1, MAX30102_MODE_CONF, 1, &mode);
}

static int set_samplingrate(max30102_t *mx, uint8_t spo2)
{
    return i2c_ioctl(mx.i2c, 1, MAX30102_SPO2_CONF, 1, &spo2);
}
...
static int set_currents(max30102_t *mx)
{
    ...
    return i2c_ioctl(mx.i2c, 1, MAX30102_LED_PULSEAMPLITUDE, 2, &amps);
}

void max_update(max30102_t *mx)
{
    unsigned char buf[4];
    i2c_ioctl(mx.i2c, 1, MAX30102_FIFO_DATA, 4, &buf);
